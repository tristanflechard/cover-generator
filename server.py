    ...
    while True:
        try:
            ...
            task.applio_output = run_model_inference(
                task.vocal_path, task.selected_model
            )
...
def split_audio(input_path):
    ...
    for output_file in output_files:
        if "vocals" in output_file.lower():
            ...
            "instrumental" in output_file.lower()
            or "accompaniment" in output_file.lower()
        ):
...
def mix_audio(instrumental_path, applio_output, input_path, selected_model):
    ...
    mix_wav_files(instrumental_path, applio_output, final_output)
...
def get_status(task_id):
    with status_lock:
        status = processing_status.get(
            task_id,
            {"step": 0, "message": "Task not found", "complete": False, "timestamp": 0},
        )

        # Add queue position information
        if status["step"] == 0 and not status["complete"]:
            queue_position = (
                list(task_queue.queue).index(
                    next(t for t in task_queue.queue if t.task_id == task_id)
                )
                + 1
            )
            status["message"] = (
                f"Your song is in queue (position: {queue_position}). It will be processed soon."
            )
    ...
    if request.method == "POST":
        ...
        if not selected_model:
            return render_template(
                "index.html", error="Please select a model", models=models
            )
        if not youtube_url:
            return render_template(
                "index.html", error="Please enter a YouTube URL", models=models
            )
